			...
			<div class='supported'>
				...
				<div class='content'>
					{#each Object.entries(PRODUCTS) as [_productId, info]}
						<img src={info.logo} title={`${_productId}`} alt="info logo">
					{/each}
				</div>
				...
				<div class='content'>
					{#each Object.entries(currencies) as [_currencyLabel, info]}
						<img src={CURRENCY_LOGOS[_currencyLabel]} title={`${formatCurrency(_currencyLabel)}`} alt="currency">
					{/each}
				</div>
				<div class='title'>Supported Collateral</div>

				<div class='content'>
					<img src={CURRENCY_LOGOS['matic']} title="MATIC" alt="matic" />
