...
<div class='pools'>
			...
			<div class='stats my-stats'>
				<div class='row'>
					<div class='label'>
						...
						{:else} -->
						<a data-intercept="true" class:disabled={!poolInfo.tvl} on:click={() => {showModal('PoolDeposit', {currencyLabel: _currencyLabel})}}>Deposit</a>
						<span class='sep'>|</span>
						<a class:disabled={poolInfo.userBalance == 0} data-intercept="true" on:click={() => {showModal('PoolWithdraw', {currencyLabel: _currencyLabel, withdrawFee: poolInfo.withdrawFee, userBalance: poolInfo.userBalance })}}>Withdraw</a>
						<!-- {/if} -->
					</div>
    ...
    <div class='pool cap-pool' class:loading={poolIsLoading['cap'] || !$capPool.supply}>
    	...
    	<div class='description'>
    		Stake Alpha-X to receive a share of revenue. <a href='#/buy'>Buy Alpha-X</a>
    	</div>
    		...
    		<div class='row'>
    			<div class='label'>
    				...
    				<!-- {#if $allowances['cap'] && $allowances['cap']['capPool'] * 1 == 0}
    					<a on:click={() => {_approveCurrency('cap')}}>Approve Alpha-X</a>
    				{:else} -->
    				<a data-intercept="true" on:click={() => {showModal('PoolDeposit', {currencyLabel: 'cap'})}}>Deposit</a>
						<span class='sep'>|</span>
						<a class:disabled={$capPool.userBalance == 0} data-intercept="true" on:click={() => {showModal('PoolWithdraw', {currencyLabel: 'cap', userBalance: $capPool.userBalance })}}>Withdraw</a>
    				<!-- {/if} -->
    			</div>
    			<div class='value'>
    				{formatToDisplay($capPool.userBalance)} Alpha-X <span class='grayed'>({formatToDisplay($capPool.supply*1 == 0 ? 0 : 100*$capPool.userBalance/$capPool.supply)}%)</span>
    			</div>
    		</div>
